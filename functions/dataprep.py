...
def create_Peak_Heights_df(PSD, dt, filename, harmonics=4, peakhalfwidth=5, feature_name='CC'):
    '''
    Extracts peak height features from a Power Spectral Density (PSD) series.
    Similar to create_Peak_Areas_df but uses the maximum value within each
    harmonic window rather than the integrated area.

    Note: this function is not currently used in any notebook. It is retained
    ...
    dt (float):             Mean time interval between measurements (seconds).
    filename (str):         Battery data file path; used to label the output row.
    harmonics (int):        Number of harmonics to extract peak heights for.
    peakhalfwidth (int):    Half-width of the window around each harmonic peak.
    feature_name (str):     Prefix for output column names ('CC' for current, 'CV' for voltage).

    Output:
    df_peak_heights (pandas DataFrame): One-row DataFrame with columns:
                                        'fundfreq{feature_name}' (fundamental frequency) and
                                        '{i:02d}_harmonic_{feature_name}heights' for each harmonic.
    '''
    # Skip the first 5 frequency bins (DC and near-DC components) to find the true fundamental.
    maxfreq_index = PSD[5:].argmax() + 5
    print(maxfreq_index)
    columns, data, harmonic_heights = [], [], []

    for harmonic in range(1, harmonics+1):
        peakhalfwidth = 10
        harmonic_heights.append(np.max(PSD[harmonic*maxfreq_index - peakhalfwidth:
                                           harmonic*maxfreq_index + peakhalfwidth]))
    print(f'harmonic_heights: {harmonic_heights}')

    Freqs = fftfreq(n=PSD.shape[0], d=dt)/(2*np.pi)

    data.append(Freqs[maxfreq_index])
    data.extend(harmonic_heights)

    columns_area = [str(i).zfill(2) + '_harmonic_' + feature_name + 'heights' for i in range(1, harmonics+1)]
    columns.append('fundfreq' + feature_name)
    columns.extend(columns_area)

    df_peak_heights = pd.DataFrame(index=columns,
                                   data=data,
                                   columns=[filename[:-4]]).T
    return df_peak_heights